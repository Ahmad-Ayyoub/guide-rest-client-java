...
from your REST service create a `src/main/java/io/openliberty/guides/consumingrest/Consumer.java`
file with `consumeWithJsonb` and `consumeWithJsonp` methods.
...
The `consumeWithJsonb` method makes a `GET` request to the running artist service
and retrieves the JSON. Then binds the JSON into an `Artist` array, use the `Artist[]`
entity type in the `readEntity` call.
